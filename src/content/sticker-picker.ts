...
export function findStickerButton(): HTMLElement | null {
  // Try to find the container with chat toolbar buttons
  ...
  // Fallback: find any button in bottom-right area that might be emoji/sticker
  const allButtons = document.querySelectorAll<HTMLElement>('[role="button"], button');
  for (const btn of allButtons) {
    const rect = btn.getBoundingClientRect();
    const label = btn.getAttribute('aria-label') || '';

    // Bottom-right of screen, reasonable size
    if (rect.bottom > window.innerHeight * 0.5 &&
        rect.right > window.innerWidth * 0.3 &&
        rect.width > 20 && rect.width < 80 &&
